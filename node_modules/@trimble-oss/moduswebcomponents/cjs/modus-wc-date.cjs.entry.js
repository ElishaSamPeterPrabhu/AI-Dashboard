...
const ModusWcDate = class {
    constructor(hostRef) {
        ...
        this.calendarMonthChange = index.createEvent(this, "calendarMonthChange");
        this.calendarYearChange = index.createEvent(this, "calendarYearChange");
        ...
        this.setupPopper = () => {
            ...
            this.popperInstance = popper.createPopper(this.inputRef, this.calendarRef, {
    ...
    renderCalendarHeader() {
        ...
        return (index.h("div", { class: "calendar-header" }, index.h("modus-wc-button", { type: "button", "aria-label": "Previous", variant: "borderless", shape: "circle", size: "xs", onButtonClick: 
            // istanbul ignore next (unreachable code)
            () => this.addMonthOffset(-1), class: "nav-btn" }, index.h("modus-wc-icon", { name: "chevron_left", size: "sm" })), index.h("div", { class: "calendar-selects" }, index.h("modus-wc-select", { key: `month-${currentYear}-${currentMonth}`, class: "month-select", value: currentMonth.toString(), options: monthOptions, onInputChange: 
            // istanbul ignore next (unreachable code)
            (e) => this.handleMonthChange(e), onInputBlur: 
            // istanbul ignore next (unreachable code)
            (e) => e.stopPropagation(), bordered: false, size: "sm" }), index.h("modus-wc-select", { key: `year-${currentYear}`, class: "year-select", value: currentYear.toString(), options: yearOptions, onInputChange: 
            // istanbul ignore next (unreachable code)
            (e) => this.handleYearChange(e), onInputBlur: 
            // istanbul ignore next (unreachable code)
            (e) => e.stopPropagation(), bordered: false, size: "sm" })), index.h("modus-wc-button", { type: "button", "aria-label": "Next", variant: "borderless", shape: "circle", size: "xs", onButtonClick: 
            // istanbul ignore next (unreachable code)
            () => this.addMonthOffset(1), class: "nav-btn" }, index.h("modus-wc-icon", { name: "chevron_right", size: "sm" }))));
    }
    renderCalendarBody() {
        const today = new Date();
        const selectedDate = this.parseISODate(this.value);
        const currentMonth = this.calendar.selectedMonth;
        return (index.h("div", { class: "calendar-body" }, index.h("div", { class: `calendar-days-week${this.showWeekNumbers ? ' has-week-numbers' : ''}` }, this.showWeekNumbers && index.h("div", { class: "week-number-header" }), this.calendar
            .getDaysOfWeek('default', WEEK_START_DAY_MAP[this.weekStartDay])
            .map((d) => {
            return index.h("div", { class: "day-header" }, d);
        })), index.h("div", { class: `calendar-dates${this.showWeekNumbers ? ' has-week-numbers' : ''}` }, this.calendar.dates.map((date, index$1) => {
            // Add week number at the start of each row (every 7 days)
            const weekNumberElement = this.showWeekNumbers && index$1 % 7 === 0 ? (index.h("div", { class: "week-number", "aria-label": `Week ${this.calendar.getWeekNumber(date, WEEK_START_DAY_MAP[this.weekStartDay])}` }, this.calendar.getWeekNumber(date, WEEK_START_DAY_MAP[this.weekStartDay]))) : null;
            if (!date) {
            ...
            const button = (index.h("button", { type: "button", class: {
                    'calendar-day': true,
                    'current-day': isToday,
                    selected: isSelected,
    ...
    render() {
        var _a, _b;
        return (index.h(index.Host, { key: 'ad188591ea7a87365f804252db6790e752d08391' }, this.label && (index.h("modus-wc-input-label", { key: '113a0f41188e7f0cf8eecea7916cf4903d3eacd0', forId: this.inputId, labelText: this.label, required: this.required, size: this.size })), index.h("div", { key: 'db03a6561049215aa4b29e89e0b091f348cfc283', class: "date-input-container" }, index.h("input", Object.assign({ key: '9b4f577fbf354fb3bb831d414c02282d65ffda69', ref: (el) => (this.inputRef = el), "aria-disabled": this.disabled, class: this.getClasses(), disabled: this.disabled, id: this.inputId, name: this.name, onBlur: this.handleBlur, onFocus: this.handleFocus, onInput: this.handleInput, onKeyDown: this.handleInputKeyDown, placeholder: this.effectiveFormat, readonly: this.readOnly, required: this.required, tabIndex: this.inputTabIndex, type: "text", value: this.hasFocus
                ? ((_b = (_a = this.inputRef) === null || _a === void 0 ? void 0 : _a.value) !== null && _b !== void 0 ? _b : '')
                : this.inputDisplayValue }, this.inheritedAttributes)), index.h("modus-wc-button", { key: 'beb3e7fff09a2bcd546f9c3bf8f1a495c8a51ea4', "aria-label": "Open calendar", disabled: this.disabled || this.readOnly, variant: "borderless", shape: "circle", size: "xs", color: "tertiary", class: "calendar-icon-button", onButtonClick: 
            // istanbul ignore next (unreachable code)
            () => this.toggleCalendar() }, index.h("modus-wc-icon", { key: '581777095a613a448fc2270b05af839468b1e205', name: "calendar_blank", size: "sm" }))), this.showCalendar && (index.h("div", { key: 'ad7daaa70009a975c071942f684038d9e9fa8283', ref: (el) => (this.calendarRef = el), class: `calendar-container${this.showWeekNumbers ? ' has-week-numbers' : ''}` }, this.renderCalendarHeader(), this.renderCalendarBody())), this.feedback && (index.h("modus-wc-input-feedback", { key: 'e41d0c5f861abf8d9519c0e70671a416390897d5', level: this.feedback.level, message: this.feedback.message, size: this.size }))));
    }
...
exports.modus_wc_date = ModusWcDate;
